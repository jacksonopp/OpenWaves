...
Broadcast Termination Signal: The source server admin can send a `TerminateStream` signal at any time via the admin API (`POST /admin/stations/{username}/stream/stop`). This immediately **suspends the ingest endpoint** (rejecting further segment uploads from the broadcaster with `503`) and propagates a `TerminateStream` ActivityPub activity to all relay followers. Each relay server verifies the activity came from its active source and stops its relay poller, purging buffered segments within 5 seconds. The cascade is source-initiated only — relay servers and external actors cannot trigger termination via the ActivityPub inbox. The source's ingest can be re-enabled at any time with `POST /admin/stations/{username}/stream/start`.
